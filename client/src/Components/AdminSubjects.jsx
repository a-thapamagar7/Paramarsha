...
const AdminSubjects = () => {
    ...
    const deleteSubjects = async (userId) => {
        const response = await fetch(`http://localhost:1447/api/subject/delete/${userId}`, {
            method: 'DELETE',
            headers: {
                'Content-Type': 'application/json'
            }
    ...
    const editSubjects = async (userId) => {
        console.log("hello")
        const response = await fetch(`http://localhost:1447/api/subject/update/${userId}`, {
            method: 'PATCH',
            headers: {
                'Content-Type': 'application/json'
            },
    ...
    return (
        <div className="flex pt-10 px-10 flex-col pb-10">
            <div className="text-3xl mb-10 font-bold tracking-tight font-sans"><span>Sub</span><span className="text-blue-700">jects</span></div>
            <div className={error.style}>{error.message}</div>
            <table className=" text-gray-500 text-xs font-medium rounded shadow-lg">
                <thead>
                ...
                <tbody>
                            ...
                            (!editPressed || value._id != identifyID) ?
                                <tr key={index} className={"border-y grid grid-cols-12 h-11 max-h-11 place-items-center " + `${index % 2 === 0 ? 'bg-white' : 'bg-gray-100'}`}>
                                    ...
                                    <td className="flex items-center gap-x-5 col-span-2">
                                        <button onClick={() => showEditMode(value.name, value.description, value._id)}><img className="h-6" src={editImg} /></button>
                                        <button onClick={() => { deleteSubjects(value._id) }}><img className="h-7" src={deleteImg} /></button>
                                    </td>
                                </tr>
                                :
                                <tr key={index} className={"border-y grid grid-cols-12 h-11 max-h-11 place-items-center " + `${index % 2 === 0 ? 'bg-white' : 'bg-gray-100'}`}>
                                    <td className="col-span-1">{index + 1}</td>
                                    <td className="col-span-3"><input className="w-full h-full bg-transparent outline-none" onChange={(e) => { setName(e.target.value) }} value={name} /></td>
                                    <td className="w-full relative col-span-6 px-4 py-2 overflow-hidden overflow-ellipsis whitespace-nowrap max-w-full">
                                        <input className="w-full h-full bg-transparent text-center outline-none" type="text" onChange={(e) => { setDescription(e.target.value) }} value={description} />
                                    </td>

                                    <td className="flex items-center gap-x-7 col-span-2">
                                        <button onClick={() => { editSubjects(value._id); setEditPressed(false); }}><img className="h-5" src={rightImg} /></button>
                    ...
                    (
                        <tr  className="border-y grid grid-cols-12 h-11 max-h-11 place-items-center">
                            ...
                            <td className="col-span-3 w-full h-full border-r"><input className="w-full h-full bg-transparent outline-none" onChange={(e) => { setName(e.target.value) }} value={name} /></td>
                            ...
                            <td className="flex items-center gap-x-7 col-span-2">
                                <button onClick={() => { createSubjects(); setName(""); setDescription(""); setCreatePressed(false); }}><img className="h-5" src={rightImg} /></button>
            ...
            <button className="bg-gray-400 w-8 h-8 mt-8 text-white" onClick={() => setCreatePressed(true)}>+</button>
