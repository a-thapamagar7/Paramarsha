...
const Quiz = () => {
    ...
    useEffect(()=>{
        getDataInfo(setOptions, "http://localhost:1447/api/getquizsubjects", )
    },[])

    const nextQuestion = async(num, value) => {
        if(num === 1){
            
            const response = await fetch(`http://localhost:1447/api/getquizcourses/${value}`,
                {
                    method: "GET",
                    headers: {
        ...
        if(num === 2)
        {
            const filteredColleges = college.filter(coll => coll.courses.includes(value))
        ...
        <div>
            <div className="bg-blue-300 h-screen flex justify-center items-center">
                <div className="bg-white h-5/6 w-2/3 rounded-lg flex flex-col items-center justify-center shadow-2xl drop-shadow-xl">
                    {found? <></>:<img className="h-48" src={imageUrls[currentIndex]} />}
                    <div className="font-bold text-xl lato">{qID != question.length-1? (qID+1) + ") ":"" }{question[qID]}</div>
                    {found? 
                        <div className="w-5/6 mt-10 flex flex-col gap-y-5">
                            <InfoCard key={"ji"} name={college[0].name} imgURL={college[0].imgURL} location={college[0].location} description={college[0].description} subjects={college[0].subjects}/>
                            <button className="bg-blue-700  rounded text-white w-1/3 h-10" onClick={() => navigate("/")}>Home</button>
                        </div>
                        :
                        <>
                        ...
                        {lastQuestion?
                            <div className="flex flex-col col-span-6 justify-center items-center gap-y-5">
                                <input className="border-2 w-96 ml-10 border-black h-10 px-3 py-2" value={collegeFee} onChange={(e)=>{setCollegeFee(e.target.value)}} type="number"/>
                                <button onClick={()=>OnSubmit()} className="bg-blue-700  rounded-sm text-white w-1/3 h-8">Submit</button>
                            ...
                            <>
                                {options.map((value, index)=>{
                                    return(
                                        <div onClick={()=>{setSelectedValue(value); nextQuestion(qID+1, value)}} key={index + value} className="quizCard cursor-pointer col-span-2 flex h-20 w-60 text-center justify-center items-center border-2 border-gray-200 rounded shadow-lg">{value}</div>
                                    )
