...
const AdminPayments = () => {
    ...
    const getPayments = async () => {
        const response = await fetch("http://localhost:1447/api/getpaymentinfo", {
            method: "GET",
            headers: {
    ...
    const deletePayments = async (userId) => {
        const response = await fetch(`http://localhost:1447/api/payment/delete/${userId}`, {
            method: 'DELETE',
            headers: {
                'Content-Type': 'application/json'
            }
    ...
    return (
        <div className="flex flex-row w-full">
            ...
            <div className="flex w-full pt-10 px-10 flex-col pb-10">
                <div className="text-2xl mb-10 font-bold tracking-tight spacegrotesk text-gray-500"><span>Payments</span></div>
                        ...
                        {payment.map((value, index) => {
                            return (
                                <tr key={index} className={"hover:bg-yellow-100 border-y grid grid-cols-12 h-11 max-h-11 place-items-center " + `${index % 2 === 0 ? 'bg-white' : 'bg-gray-100'}`}>
                                    ...
                                    <td className="col-span-2">{value.user.firstName + " "}{value.user.lastName}</td>
                                    ...
                                    <td className="flex items-center gap-x-5 col-span-1">
                                        <button onClick={() => { deletePayments(value._id) }}><img className="h-7" src={deleteImg} /></button>
                                    </td>
                                </tr>
                            )
