...
const CreateUniversity = () => {
  ...
  const getIDUniversities = async () => {
    const response = await fetch(
      `${process.env.REACT_APP_API_URL}/api/getrequireduniversity/${id}`,
      {
        method: "GET",
        headers: {
  ...
  const editUniversity = async (userId) => {
    const response = await fetch(
      `${process.env.REACT_APP_API_URL}/api/university/update/${id}`,
      {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
      ...
      <div className="px-20">
        ...
        >
          <div className=" text-4xl tracking-tighter font-sans font-bold">
            <span>Add New </span>
            <span className="text-blue-700">University</span>
          </div>
              ...
              return (
                ...
                >
                  <input
                    ...
                    type="checkbox"
                    value={v.name}
                    checked={colleges.indexOf(v.name) !== -1}
                    onChange={handleCheckboxCollege}
                  />
                  <label htmlFor={v.name + i} className="text-base">
                    {capitalizeFirstLetter(v.name)}
                  </label>
