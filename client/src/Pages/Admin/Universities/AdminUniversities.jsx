...
const AdminUniversties = () => {
    ...
    const deleteUniversities = async (userId) => {
        const response = await fetch(`http://localhost:1447/api/university/delete/${userId}`, {
            method: 'DELETE',
            headers: {
                'Content-Type': 'application/json'
            }
    ...
    return (
        <div className="flex flex-row w-full">
            ...
            <div className="flex w-full pt-10 px-10 flex-col pb-10">
                <div className="text-2xl mb-10 font-bold tracking-tight spacegrotesk text-gray-500"><span>Universities</span></div>
                        ...
                        {university.map((value, index) => {
                            return (
                                <tr key={index} className={"border-y hover:bg-yellow-100 grid grid-cols-12 h-11 max-h-11 place-items-center " + `${index % 2 === 0 ? 'bg-white' : 'bg-gray-100'}`}>
                                    ...
                                    <td className="gap-x-1 col-span-2 px-4 py-2 overflow-hidden overflow-ellipsis whitespace-nowrap max-w-full">{value.location}</td>
                                    ...
                                    <td className="flex items-center gap-x-5 col-span-1">
                                        <button onClick={()=>{navigate(`/admin/universities/add/${value._id}`)}}><img className="h-6" src={editImg} /></button>
                                        <button onClick={()=>{deleteUniversities(value._id)}}><img className="h-7" src={deleteImg} /></button>
                                    </td>
                                </tr>
                            )
                        })}
                ...
                <button className="bg-gray-400 w-8 h-8 mt-8 text-white hover:bg-gray-700" onClick={()=>{navigate("/admin/universities/add")}}>+</button>
