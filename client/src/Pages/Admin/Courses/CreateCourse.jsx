...
const CreateCourse = () => {
    ...
    const editCourse = async (userId) => {
        const response = await fetch(`http://localhost:1447/api/course/update/${id}`, {
            method: 'PATCH',
            headers: {
                'Content-Type': 'application/json'
            },
    ...
    const getIDCourses = async () => {
        const response = await fetch(`http://localhost:1447/api/getrequiredcourse/${id}`, {
            method: "GET",
            headers: {
                "Content-Type": "application/json"
            }
    ...
    const createCourses = async () => {
        const response = await fetch("http://localhost:1447/api/course/create", {
            method: "POST",
            ...
            <div className="px-20">
                ...
                <form onSubmit={submitCourse} className="flex flex-col mt-10 gap-y-6 lato">
                    ...
                    <div className="flex flex-row gap-x-10">
                        <button type="submit" className="border bg-blue-700 text-white w-1/12 spacegrotesk h-12">{id? (<>Update</>):(<>Add</>)}</button>
                        <button onClick={()=>navigate("/admin/courses")} className="border bg-blue-700 text-white w-1/12 spacegrotesk h-12">Cancel</button>
                    </div>
