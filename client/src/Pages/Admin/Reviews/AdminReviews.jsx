...
const AdminReviews = () => {
  ...
  const getReviews = async () => {
    const response = await fetch(
      `${process.env.REACT_APP_API_URL}/api/getreviewinfo`,
      {
        method: "GET",
        headers: {
  ...
  const deleteReviews = async (userId) => {
    const response = await fetch(
      `${process.env.REACT_APP_API_URL}/api/review/delete/${userId}`,
      {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
            ...
            {review.map((value, index) => {
              return (
                ...
                >
                  ...
                  <td className="col-span-1 flex flex-col items-center">
                  ...
                  <td className="col-span-2 max-w-full overflow-hidden whitespace-nowrap truncate h-full hover:whitespace-nowrap hover:trunca hover:overflow-x-scroll">
                    {value.comment}
                  </td>
                  <td className="flex items-center gap-x-5 col-span-1">
                    <button
                      onClick={() => {
                        deleteReviews(value._id);
                      }}
                    >
