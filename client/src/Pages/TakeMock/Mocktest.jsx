...
const Mocktest = () => {
  ...
  const getSubjects = async () => {
    const response = await fetch(
      `${process.env.REACT_APP_API_URL}/api/getquestions`,
      {
        method: "GET",
        headers: {
  ...
  const handleAnswerSelect = (questionID, answer) => {
    ...
    if (question.answer !== answer) {
      ...
      if (newWrongAnswers.find((q) => q.q_ID === questionID)) {
        const index = newWrongAnswers.indexOf(
          newWrongAnswers.find((q) => q.q_ID === questionID)
        );
    ...
    // to remove the wrong response if the user has corrected it
      ...
      const i = newWrongAnswers.indexOf(
        newWrongAnswers.find((q) => q.q_ID === questionID)
      );
  ...
  const checkAnswers = () => {
    ...
    for (let i = 0; i < Object.keys(selectedAnswers).length; i++) {
      ...
      const question = questions.find(
        (q) => q._id === Object.keys(selectedAnswers)[i]
      );
  ...
  const onFinished = async (event) => {
    ...
    if (
      Object.keys(selectedAnswers).length + wrongAnswers.length !==
      questions.length
    ) {
            ...
            {finished ? (
              ...
              <div className="w-full flex-col flex justify-center mt-14">
                <div className="flex flex-col gap-y-4 text-center font-sans tracking-tighter w-full">
                  ...
                  <div className="text-xl justify-center w-full">
                    {score} out of {questions.length} answers were correct in{" "}
                    {selectedSubject}
                  </div>
                </div>
              ...
              <form
                onSubmit={onFinished}
                className="flex flex-col gap-y-11 mt-10 pl-5"
              >
                <div className="w-full text-center text-2xl tracking-tighter">
                  {capitalizeFirstLetter(selectedSubject)}
                </div>
                {questions.map((question, i) => (
                  <Question
                    ...
                    question={question}
                    questionNo={i + 1}
                    nameGroup={question._id}
                    handleOption={handleAnswerSelect}
                  />
                ))}
                <button
                  className="w-36 h-11 bg-blue-700 text-white spacegrotesk"
                  type="submit"
                >
                  Submit
                </button>
          ...
          <div className="flex justify-center items-center mt-20">
            <div className="p-5 w-1/2 flex flex-col gap-y-12">
              ...
              <form
                ...
                className="flex flex-col gap-x-4 gap-y-9 text-lg"
              >
                ...
                <div className="flex justify-between gap-x-5">
                  ...
                  <input
                    type="number"
                    className="border border-black py-2 px-3 w-80"
                    value={questionNos}
