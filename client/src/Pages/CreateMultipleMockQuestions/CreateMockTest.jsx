...
const CreateMockTest = () => {
  ...
  const createMultiQuestion = async (multiArray) => {
    const response = await fetch(
      `${process.env.REACT_APP_API_URL}/api/createquestion/many`,
      {
        method: "POST",
  ...
  const createQuestion = async () => {
    const response = await fetch(
      `${process.env.REACT_APP_API_URL}/api/createquestion/one`,
      {
        method: "POST",
        //sends the data in json format
        headers: {
        ...
        {!submittedMethod ? (
          <div className="flex justify-center items-center mt-20">
            <div className="p-5 w-1/2 flex flex-col gap-y-12">
              <div className="text-4xl font-sans font-extrabold tracking-tight text-gray-900 w-full flex">
                Create Mock Questions
              </div>
              <form
                onSubmit={OnStartSubmit}
                className="flex flex-col gap-x-4 gap-y-7 text-lg"
              >
                ...
                <button
                  type="submit"
                  className="border w-2/6 h-10 bg-blue-700 spacegrotesk text-sm text-white"
                >
                  Start
                </button>
              </form>
            ...
            {selectedValue == "many" ? (
              <div className="grid grid-cols-1 gap-y-5 mb-5">
                <div className="text-3xl font-sans font-extrabold tracking-tight">
                  Create Mock <span className="text-blue-700">Test</span>
                </div>
                <form
                  onSubmit={OnMultiSubmit}
                  className="grid grid-cols-1 gap-y-5 mb-20"
                >
                  <div className="flex flex-row gap-x-7 items-center">
                    <label className={forHeading + " font-extrabold"}>
                      Subject:
                    </label>
                    <input
                      value={subject}
                      placeholder="Subject"
                      onChange={onSubject}
                      className="border-black text-base borderM question1 w-4/12 py-3 px-6"
                    />
                  </div>
                  <hr className="mb-10" />
                  <div>
                    <div className={forHeading + " font-extrabold"}>
                      Instructions:
                    </div>
                    <ul className="list-disc">
                      <li>
                        Each individual question should be in different lines.
                      </li>
                      <li>
                        Use ; as seperator between questions, options and answer
                        without the use of space.
                      </li>
                      <li>
                        Denote question with (que) at the begining with no
                        space, (ans) for answer and (opt) for options at the
                        begining with no spaces
                      </li>
                    </ul>
                    <div className="flex flex-col mt-5 mb-5">
                      <span className={forHeading + " font-extrabold"}>
                        Demo
                      </span>
                      <span>
                        (que)What is
                        earth?;(ans)planet;(opt)star;(opt)satellite;(opt)galaxy
                      </span>
                      <span>
                        (que)Which planet is called the red
                        planet?;(ans)mars;(opt)earth;(opt)venus;(opt)jupiter
                      </span>
                    </div>
                  </div>
                  <textarea
                    value={multiQuestions}
                    onChange={onMultiQuestions}
                    className="flex text-sm border-black borderM question w-full h-96 py-3 px-6"
                  />
                  <button
                    type="submit"
                    className="border w-2/12 h-14  bg-blue-700 spacegrotesk text-white mt-6 text-sm"
                  >
                    Submit
                  </button>
              ...
              <>
                <form
                  ...
                  className="mt-14 flex flex-col gap-y-14"
                >
                  <div className="text-3xl font-sans font-extrabold tracking-tight">
                    Create Mock <span className="text-blue-700">Test</span>
                  </div>
                  <div className="grid grid-cols-1 gap-y-5 mb-20">
                    ...
                    <hr className="mb-10" />
                    <label
                      className={forHeading + " underline underline-offset-2"}
                    >
                      Question
                    </label>
                    <textarea
                      value={question}
                      onChange={onQuestion}
                      className="flex text-sm border-black border-2 question w-full h-28 py-3 px-6"
                    />
                    <div className="flex flex-row gap-x-5 items-center">
                      <label className={forHeading + " text-blue-700"}>
                        Answer:
                      </label>
                      <input
                        value={answer}
                        onChange={onAnswer}
                        placeholder="Answer"
                        className="border-black text-sm borderM question1 w-8/12 py-3 px-6"
                      />
                    </div>
                    <div className="flex flex-row gap-x-6 items-center">
                      ...
                      <input
                        value={options[0]}
                        onChange={(e) => onOption(0, e)}
                        placeholder="Option"
                        className="border-black text-sm borderM question1 w-1/2 py-3 px-6"
                      />
                    </div>
                    <div className="flex flex-row gap-x-6 items-center">
                      <label className={forHeading + ""}>Option:</label>
