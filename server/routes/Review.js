...
router.get("/getcolleges/:collegeID", async (req, res) => {
  ...
  Review.find({ college: collegeID })
    ...
    .exec((err, reviews) => {
      ...
      const facilitiesAvg = Math.trunc(
        reviews.reduce((acc, cur) => acc + cur.facilities, 0) / numReviews
      );
      const educationAvg = Math.trunc(
        reviews.reduce((acc, cur) => acc + cur.education, 0) / numReviews
      );
      const overallRatingAvg = Math.trunc(
        reviews.reduce((acc, cur) => acc + cur.overallRating, 0) / numReviews
      );
      ...
      return res.json({
        ...
        data: reviews,
        allReviews: { facilitiesAvg, educationAvg, overallRatingAvg },
      });
    });
});
